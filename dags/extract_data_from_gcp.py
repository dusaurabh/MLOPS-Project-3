...
def load_to_sql(file_path):
    ...
    engine = sqlalchemy.create_engine(f"postgresql+psycopg2://{conn.login}:{conn.password}@ml-ops-project-3_e97fc1-postgres-1:{conn.port}/{conn.schema}")
...
with DAG(
    ...
    list_files = GCSListObjectsOperator(
        task_id="list_files",
        bucket="saurabh-mlops-project-3-bucket", 
    )
